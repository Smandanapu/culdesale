...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#07090e] text-slate-900 dark:text-slate-100 overflow-x-hidden relative font-sans">
      ...
      <nav className="sticky top-0 z-50 bg-slate-50 dark:bg-[#07090e]/75 backdrop-blur-md border-b border-slate-200 dark:border-white/[0.06] px-6 py-4 flex items-center justify-between relative">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <button onClick={() => navigate('/garage-sales')} className="sm:hidden px-2 text-xl hover:scale-110 transition duration-200 cursor-pointer" title="Garage Sales">
            🏷️
          </button>
      ...
      <div className="relative z-40 border-b border-slate-200 dark:border-white/[0.04] bg-slate-50 dark:bg-[#07090e]/50 backdrop-blur-sm py-3.5 overflow-hidden w-full select-none">
        <div className="flex animate-marquee gap-16 whitespace-nowrap">
          {/* First loop of items */}
          <div className="flex gap-16 text-xs text-slate-500 dark:text-slate-400 font-semibold tracking-wide shrink-0">
            <span>🛋️ Velvet Sectional sold for $180 in Sector B-3</span>
            <span className="text-slate-900 dark:text-white/[0.1]">·</span>
          ...
          <div className="flex gap-16 text-xs text-slate-500 dark:text-slate-400 font-semibold tracking-wide shrink-0" aria-hidden="true">
            <span>🛋️ Velvet Sectional sold for $180 in Sector B-3</span>
            <span className="text-slate-900 dark:text-white/[0.1]">·</span>
            <span>🚲 Specialized Cruiser Bike has 4 active bids in Sector C</span>
            <span className="text-slate-900 dark:text-white/[0.1]">·</span>
      ...
      <div className="relative z-10 px-6 py-24 md:py-32 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          <div className="lg:col-span-7 flex flex-col items-center lg:items-start text-center lg:text-left">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold tracking-tight leading-[1.1] mb-6 text-slate-900 dark:text-white">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-base md:text-lg max-w-xl mb-10 leading-relaxed font-normal">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
              <button onClick={() => navigate('/register')} className="w-full sm:w-auto px-8 py-4.5 bg-gradient-to-r from-orange-500 via-orange-600 to-amber-500 text-slate-900 dark:text-white text-lg font-bold rounded-2xl shadow-xl shadow-orange-500/15 hover:shadow-2xl hover:shadow-orange-500/30 hover:scale-[1.03] active:scale-[0.98] transition duration-300 cursor-pointer text-center">Start Selling Free →</button>
              <button onClick={() => navigate('/login')} className="w-full sm:w-auto px-8 py-4.5 bg-white dark:bg-white/[0.03] hover:bg-white/[0.06] border border-slate-200 dark:border-white/[0.08] text-slate-900 dark:text-white text-lg font-bold rounded-2xl transition duration-300 text-center cursor-pointer">Browse Feed</button>
            </div>
          ...
          <div className="lg:col-span-5 w-full max-w-md mx-auto relative z-20">
            ...
            <div className={`relative rounded-3xl backdrop-blur-xl border p-3 sm:p-6 transition-all duration-300 shadow-2xl ${
              ...
              : 'border-slate-200 dark:border-white/[0.08] bg-white dark:bg-white/[0.02] hover:border-white/[0.12]'
            }`}>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] text-[10px] font-bold text-slate-500 dark:text-slate-400">📍 Sector B-12</span>
              </div>
              <div className="relative rounded-2xl overflow-hidden aspect-[4/3] mb-4 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-white/[0.06]">
                ...
                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-[#07090e]/85 backdrop-blur-md border border-slate-200 dark:border-white/[0.08]">
                  <div>
                    <h4 className={`text-xs font-bold text-slate-900 dark:text-white leading-tight transition-opacity duration-300 ${demoFade ? 'opacity-100' : 'opacity-0'}`}>{demoItems[demoIndex].title}</h4>
                    <p className={`text-[10px] text-slate-500 dark:text-slate-400 leading-tight transition-opacity duration-300 ${demoFade ? 'opacity-100' : 'opacity-0'}`}>Offered by neighbor {demoItems[demoIndex].seller}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[8px] text-slate-500 uppercase tracking-widest font-bold">Start</p>
                    <p className={`text-xs font-bold text-slate-600 dark:text-slate-300 transition-opacity duration-300 ${demoFade ? 'opacity-100' : 'opacity-0'}`}>{demoItems[demoIndex].start}</p>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 p-4 rounded-2xl bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.04] mb-5">
                <div>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">Current Bid</p>
                  <div className={`text-3xl font-extrabold leading-none transition-all duration-300 ${flashType === 'user' ? 'text-emerald-400' : flashType === 'neighbor' ? 'text-orange-400' : 'text-slate-900 dark:text-white'}`}>${currentBid}.00</div>
                </div>
                <div>
              ...
              <div className="mb-5">
                ...
                <div className="space-y-2 max-h-[140px] overflow-hidden">
                  {bids.map((bid, i) => (
                    <div key={bid.id} className={`flex items-center justify-between p-2 rounded-xl border transition-all duration-300 ${i === 0 ? bid.name === 'You' ? 'bg-emerald-950/20 border-emerald-500/20' : 'bg-orange-950/15 border-orange-500/20' : 'bg-white/[0.01] border-slate-200 dark:border-white/[0.04]'} ${i === 0 && bidTrigger ? 'scale-[1.01]' : ''}`}>
                      <div className="flex items-center gap-2.5">
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-slate-900 dark:text-white ${bid.color}`}>{bid.avatar}</div>
                        <div>
                      ...
                      <div className="text-right">
                        <p className={`text-sm font-extrabold ${i === 0 ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>${bid.amount}.00</p>
              ...
              <button onClick={handleUserBid} className="w-full py-3.5 bg-gradient-to-r from-orange-500 via-orange-600 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-slate-900 dark:text-white font-extrabold rounded-2xl shadow-lg shadow-orange-500/10 hover:shadow-xl hover:shadow-orange-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 cursor-pointer flex items-center justify-center gap-2 group text-sm">
      ...
      <div className="px-6 py-24 border-t border-slate-200 dark:border-white/[0.06] relative z-10 max-w-6xl mx-auto">
        ...
        <p className="text-center text-slate-500 dark:text-slate-400 text-sm md:text-base max-w-md mx-auto mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {[
            ...
            <div 
              key={item.step} 
              className="card-gradient-border group relative flex flex-col items-center text-center p-8 rounded-3xl bg-white/[0.012] backdrop-blur-md transition-all duration-300 hover:bg-white dark:bg-white/[0.03] hover:-translate-y-2 hover:shadow-2xl hover:shadow-orange-500/5 cursor-default"
            >
              ...
              <h3 className="text-xl font-bold mb-2 text-slate-900 dark:text-white relative z-10">{item.title}</h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed relative z-10">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>

      <div className="px-6 py-20 border-t border-b border-slate-200 dark:border-white/[0.06] bg-white/[0.01] backdrop-blur-md relative z-10">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 text-center">
          {[
            ...
            <div key={f.label} className="flex flex-col items-center gap-1 sm:gap-2 group cursor-default">
              ...
              <div className="text-sm sm:text-base font-bold text-slate-900 dark:text-white tracking-tight">{f.label}</div>
      ...
      <div id="safety" className="px-6 py-24 border-b border-slate-200 dark:border-white/[0.06] relative z-10 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <div key={i} className="card-gradient-border group relative flex flex-col p-6 rounded-3xl bg-white dark:bg-white/[0.015] backdrop-blur-md transition-all duration-300 hover:bg-white dark:bg-white/[0.03] hover:-translate-y-1">
              ...
              <h3 className="text-lg font-bold mb-2 text-slate-900 dark:text-white relative z-10">{item.title}</h3>
              <p className="text-slate-500 dark:text-slate-400 text-xs leading-relaxed relative z-10">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>

      <div className="px-6 py-24 text-center relative z-10 max-w-4xl mx-auto overflow-hidden rounded-3xl my-16 border border-slate-200 dark:border-white/[0.06] bg-white/[0.01] backdrop-blur-sm shadow-2xl">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base max-w-md mx-auto mb-10 relative z-10">
          Join your neighborhood on CulDeSale today. Start decluttering and bidding.
        </p>
        <button onClick={() => navigate('/register')} className="px-8 py-4 bg-gradient-to-r from-orange-500 to-amber-500 text-slate-900 dark:text-white text-lg font-bold rounded-2xl shadow-lg shadow-orange-500/10 hover:shadow-xl hover:shadow-orange-500/25 hover:scale-[1.03] active:scale-[0.98] transition duration-300 relative z-10 cursor-pointer">Join Free →</button>
      </div>
      ...
      <div className="px-6 py-8 border-t border-slate-200 dark:border-white/[0.04] relative z-10 flex flex-col md:flex-row items-center justify-between max-w-6xl mx-auto gap-4 text-slate-500 text-sm">
        ...
        <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-xs font-medium">
          <Link to="/privacy" className="hover:text-slate-900 dark:text-white transition duration-200">Privacy Policy</Link>
          <span className="hidden sm:inline">·</span>
          <Link to="/terms" className="hover:text-slate-900 dark:text-white transition duration-200">Terms of Service</Link>
          <span className="hidden sm:inline">·</span>
          <Link to="/contact" className="hover:text-slate-900 dark:text-white transition duration-200">Contact</Link>
        </div>
      </div>
    </div>
  )
