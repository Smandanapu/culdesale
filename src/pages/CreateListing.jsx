    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#07090e] bg-grid-pattern text-slate-900 dark:text-slate-100 relative overflow-hidden">
      ...
      <div className="max-w-lg mx-auto px-4 py-8 relative z-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">List an Item</h1>
          <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] px-2.5 py-1 rounded-md">Step {step} of 3</span>
        </div>

        <div className="w-full bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] rounded-full h-1.5 mb-8 overflow-hidden backdrop-blur-md">
          <div
        ...
        <div className="card-gradient-border bg-white dark:bg-white/[0.015] backdrop-blur-md border border-slate-200 dark:border-white/[0.04] rounded-2xl p-6 shadow-2xl">
          {step === 1 && (
            <div className="flex flex-col gap-5">
              <div className="flex items-center justify-between mb-1">
                <div className="text-lg font-bold text-slate-900 dark:text-white">Item Details</div>
                {aiStatus === 'analyzing' && (
              ...
              <div>
                ...
                {form.photos.length < 5 && (
                  <label className="flex flex-col items-center justify-center h-28 border-2 border-dashed border-slate-200 dark:border-white/[0.08] hover:border-orange-500/60 rounded-xl cursor-pointer hover:bg-white dark:bg-white/[0.02] transition-all duration-300">
                    {uploading ? (
          ...
          {step === 2 && (
            <div className="flex flex-col gap-5">
              <div className="text-lg font-bold text-slate-900 dark:text-white mb-1">Pricing & Duration</div>
              ...
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2 block">Auction Duration</label>
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 rounded-xl text-xs font-bold transition-all duration-200 border cursor-pointer ${
                        form.duration.label === d.label
                          ...
                          : 'bg-white dark:bg-white/[0.02] border-slate-200 dark:border-white/[0.06] text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-white/[0.05]'
                      }`}
                    >{d.label}</button>
                  ))}
                </div>
          ...
          {step === 3 && (
            <div className="flex flex-col gap-4">
              <div className="text-lg font-bold text-slate-900 dark:text-white mb-1">Meetup Preference</div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {MEETUP_TYPES.map(m => (
                  ...
                  >
                    ...
                    <span className={`text-xs font-bold ${form.meetup_type === m ? 'text-orange-400' : 'text-slate-600 dark:text-slate-300'}`}>{m}</span>
                  </div>
                ))}
              </div>
          ...
          <div className="flex gap-3 mt-8 border-t border-slate-200 dark:border-white/[0.04] pt-6">
            {step > 1 && (
              <button
                onClick={() => setStep(s => s - 1)}
                className="flex-1 py-3 bg-white dark:bg-white/[0.02] hover:bg-white/[0.05] border border-slate-200 dark:border-white/[0.06] text-slate-900 dark:text-white rounded-xl font-bold transition-all active:scale-[0.98] cursor-pointer"
              >
                Back
              </button>
            )}
            {step < 3 ? (
              <button
                ...
                className="flex-1 py-3 bg-gradient-to-r from-orange-500 to-amber-500 hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed text-slate-900 dark:text-white rounded-xl font-bold transition-all active:scale-[0.98] cursor-pointer shadow-lg shadow-orange-500/25"
              >
                Continue
              </button>
            ) : (
