...
export default function EditGarageSale() {
  ...
  const handleSubmit = async (e) => {
    ...
    let uploadedModelUrl = undefined
    if (modelFile) {
    ...
    if (uploadedModelUrl !== undefined) {
      updatePayload.model_url = uploadedModelUrl
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-2xl p-5 sm:p-6 shadow-sm">
            ...
            <div className="bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-xl p-4 mb-4 text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
              <strong>Update your 3D Scan!</strong> You can replace the 3D scan of your garage sale so buyers can virtually explore your items before driving over. 
              <br/><br/>
            ...
            <div>
              <label className="text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5 block">Upload New 3D Model File (.glb, .gltf) - Optional</label>
              <input
                type="file"
                accept=".glb,.gltf"
          ...
          >
            {uploadingModel ? '⏳ Uploading 3D Model...' : loading ? '⏳ Saving...' : '💾 Save Changes'}
          </button>
        </form>
      </div>
    </div>
